...
# clase: Basico::CausaDevolucionCliente
# descripción: Migración a Rails 3
#


class CausaDevolucionCliente < ActiveRecord::Base

  self.table_name = 'causa_devolucion_cliente'
  ...
  validates_length_of :nombre, :within => 1..80, :allow_nil => false,
  ...
  def validate
    unless self.nombre.nil? || self.nombre.empty?      
      ...
      if a.nil?        
        errors.add(:causa_devolucion_cliente, I18n.t('Sistema.Body.Modelos.Actividad.Errores.nombre_invalido'))
      end
    end
    unless self.descripcion.nil? || self.descripcion.empty?
      a = self.descripcion[/^[0-9a-zA-Z°;:,.\/ñÑáÁéÉíÍóÓúÚüÜÇ' ]+$/]
      if a.nil?
        errors.add(:causa_devolucion_cliente, I18n.t('Sistema.Body.Modelos.Actividad.Errores.descripcion_invalida'))
      end
    end 
  end
...
# == Schema Information
#
# Table name: causa_devolucion_cliente
#
