...
# clase: Basico::CausaRechazo
# descripción: Migración a Rails 3
#


class CausaRechazo < ActiveRecord::Base

  self.table_name = 'causa_rechazo'
  ...
  def validate
    unless self.nombre.nil? || self.nombre.empty?
      ...
      if a.nil?
        errors.add(:causa_rechazo, I18n.t('Sistema.Body.Modelos.Actividad.Errores.nombre_invalido'))
      end
    end
    unless self.descripcion.nil? || self.descripcion.empty?
      a = self.descripcion[/^[0-9a-zA-Z°;:,.\/ñÑáÁéÉíÍóÓúÚüÜÇ' ]+$/]
      if a.nil?
        errors.add(:causa_rechazo, I18n.t('Sistema.Body.Modelos.Actividad.Errores.descripcion_invalida'))
      end
    end 
  end
