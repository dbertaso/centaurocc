...
#
# autor: Diego Bertaso
# clase: Basico::ConfiguracionAvanceController
# descripción: Build in Rails 3
#
class Basico::ConfiguracionAvanceController < FormAjaxController
  ...
  def list
    ...
    @list = ConfiguracionAvance.search(@condition, 
                                      params[:page], 
                                      params['sort'])
  ...
  def new
    fill
    @configuracion_avance = ConfiguracionAvance.new
		form_new @configuracion_avance
  end
  ...
  def save_new
    @configuracion_avance = ConfiguracionAvance.new(params[:configuracion_avance])
		form_save_new @configuracion_avance
  end
  
  def delete
    @configuracion_avance = ConfiguracionAvance.find(params[:id])
		form_delete @configuracion_avance
  end
  
  def edit
    fill
    @configuracion_avance = ConfiguracionAvance.find(params[:id])
		form_edit @configuracion_avance
  end
  def save_edit
    @configuracion_avance = ConfiguracionAvance.find(params[:id])
		form_save_edit @configuracion_avance
  end
  def cancel_edit
    @configuracion_avance = ConfiguracionAvance.find(params[:id])
		form_cancel_edit @configuracion_avance
  end

  protected
  def common
    super
    @form_title = I18n.t('Sistema.Body.Controladores.ConfiguracionAvance.FormTitles.form_title')
    @form_title_record = I18n.t('Sistema.Body.Controladores.ConfiguracionAvance.FormTitles.form_title')
    @form_title_records = I18n.t('Sistema.Body.Controladores.ConfiguracionAvance.FormTitles.form_title_records')
    @form_entity = 'configuracion_avance'
