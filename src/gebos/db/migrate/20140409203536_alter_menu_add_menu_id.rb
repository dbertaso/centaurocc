class AlterMenuAddMenuId < ActiveRecord::Migration
  def up
  	execute "DROP VIEW view_recuperacion_cartera;
...
SELECT solicitud.numero AS numero_tramite, prestamo.numero AS numero_financiamiento, 
        ...
        CASE
            ...
            ELSE empresa.rif
        END AS documento_beneficiario, estado.nombre AS estado_nombre, estado.id AS estado_id, solicitud.sector_id, sector.nombre AS sector_nombre, solicitud.sub_sector_id, sub_sector.nombre AS sub_sector_nombre, solicitud.rubro_id, rubro.nombre AS rubro_nombre, solicitud.sub_rubro_id, sub_rubro.nombre AS sub_rubro_nombre, solicitud.actividad_id, actividad.nombre AS actividad_nombre, solicitud.programa_id, programa.nombre AS programa_nombre, prestamo.fecha_liquidacion, plan_pago.fecha_fin AS fecha_vencimiento, solicitud.monto_aprobado, prestamo.monto_banco, prestamo.monto_insumos, prestamo.monto_liquidado AS monto_liquidado_banco, prestamo.monto_banco - prestamo.monto_liquidado AS monto_por_liquidar_banco, solicitud.por_inventario, prestamo.monto_inventario, prestamo.monto_despachado AS monto_despachado_insumos, prestamo.monto_insumos - prestamo.monto_despachado AS monto_por_despachar_insumos, prestamo.estatus AS estatus_financiamiento, prestamo.cantidad_cuotas_vencidas, prestamo.capital_vencido, prestamo.interes_vencido, prestamo.monto_mora, prestamo.saldo_insoluto AS saldo_deudor, prestamo.exigible AS monto_exigible, prestamo.deuda AS deuda_total, prestamo.capital_pagado AS capital_recuperado, prestamo.intereses_pagados AS interes_recuperado, prestamo.mora_pagada AS interes_mora_pagada, prestamo.remanente_por_aplicar AS excedente_por_pagar, plan_pago_cuota.fecha AS fecha_cuota, plan_pago_cuota.amortizado AS capital_recuperar, plan_pago_cuota.interes_corriente + plan_pago_cuota.interes_diferido AS interes_recuperar, 
        CASE
            WHEN vepb.monto_excedente_pagado IS NULL THEN 0::numeric
            ELSE vepb.monto_excedente_pagado
        END AS excedente_pagado, prestamo.monto_sras_total, prestamo.monto_gasto_total AS monto_gastos_total
   FROM prestamo
 ...
 ALTER TABLE menu ADD COLUMN menu_id integer;
 UPDATE menu SET menu_id = id;"
  end
