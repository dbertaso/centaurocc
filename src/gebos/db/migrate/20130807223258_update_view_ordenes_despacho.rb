...
class UpdateViewOrdenesDespacho < ActiveRecord::Migration
  def up
      ...
      CREATE OR REPLACE VIEW view_list_orden_despacho_consulta AS 
       select * from (SELECT DISTINCT ON (orden_despacho_id) ode.prestamo_id AS prestamo, cli.id AS cliente_id, fac_or_des.numero_factura, 
            CASE
      ...
      WHERE fac_or_des.confirmada = true AND fac_or_des.emitida = true and sol.estatus_id not in (10140,10150)) as tempo
    where no_va='';
    ...
    DROP VIEW IF EXISTS view_list_orden_despacho;
      ...
      select * from (SELECT s.id AS solicitud_id, 
            CASE
     ...
     LEFT JOIN prestamo prest ON prest.solicitud_id = s.id 
      ...
      WHERE (c.empresa_id IS NOT NULL OR c.persona_id IS NOT NULL) and s.estatus_id not in (10140,10150)) as tempo
    where no_va='';     
    "
  end
