-- Function: ejecutar_pago(p_cliente_id int4, p_cheques _varchar, p_prestamo_id int4, p_modalidad bpchar, p_monto float8, p_oficina_id int4, p_fecha_realizacion date, p_fecha date, p_numero_voucher "varchar", p_monto_efectivo float8, p_entidad_financiera_id int4, p_proceso_nocturno bool, p_cancelacion_prestamo bool, p_hay_cheques bool, p_exonerar_mora bool)

-- DROP FUNCTION ejecutar_pago(p_cliente_id int4, p_cheques _varchar, p_prestamo_id int4, p_modalidad bpchar, p_monto float8, p_oficina_id int4, p_fecha_realizacion date, p_fecha date, p_numero_voucher "varchar", p_monto_efectivo float8, p_entidad_financiera_id int4, p_proceso_nocturno bool, p_cancelacion_prestamo bool, p_hay_cheques bool, p_exonerar_mora bool);
...
--    if _anio_1 = 0 or _anio_1 = extract (year from _plan_pago_cuota.fecha) then
   ...
   --FIN CONTABILIDAD
	...
	/*
	------------------------------------------------
	Se incluyo este codigo para recalcular la mora
	cuando hay mas de dos cuotas vencidas y hay un
	remanente por aplicar y la fecha de realizacion
	es menor que la siguiente cuota vencida
	------------------------------------------------
	...
	*/
    ...
    /*
    ----------------------------------------------
    Se procede a recalcular la mora por reajuste 
    de pagos atrasados 
    ----------------------------------------------
    */

	_fecha_calculo = agregar_dias(p_fecha, -1);
	 perform calcular_mora(_prestamo_id, _fecha_calculo, _fecha_calculo, false);
...
ALTER FUNCTION ejecutar_pago(p_cliente_id int4, p_cheques _varchar, p_prestamo_id int4, p_modalidad bpchar, p_monto float8, p_oficina_id int4, p_fecha_realizacion date, p_fecha date, p_numero_voucher "varchar", p_monto_efectivo float8, p_entidad_financiera_id int4, p_proceso_nocturno bool, p_cancelacion_prestamo bool, p_hay_cheques bool, p_exonerar_mora bool) OWNER TO cartera;
