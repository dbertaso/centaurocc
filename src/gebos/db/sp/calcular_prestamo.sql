-- Function: calcular_prestamo(p_prestamo_id int4, p_fecha_evento date, p_proyeccion bool)

-- DROP FUNCTION calcular_prestamo(p_prestamo_id int4, p_fecha_evento date, p_proyeccion bool);

CREATE OR REPLACE FUNCTION calcular_prestamo(p_prestamo_id int4, p_fecha_evento date, p_proyeccion bool)
  RETURNS bool AS
$BODY$
...
begin
  ...
  if _prestamo.estatus <> 'L' then
	...
	/* comentado por Diego Bertaso para probar actualizacion de dias de mora durante el pago de prestamo 
	   ya que este proceso lo hace la rutina de calcular mora*/
...
ALTER FUNCTION calcular_prestamo(p_prestamo_id int4, p_fecha_evento date, p_proyeccion bool) OWNER TO cartera;
