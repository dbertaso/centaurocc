...
begin
	...
	loop
	    ...
	    if _tasa_valor.fecha_resolucion_hasta is not null then
	     if p_fecha_hasta > _tasa_valor.fecha_resolucion_hasta then
              _fecha_fin = _tasa_valor.fecha_resolucion_hasta + 1;
             else
  ...
  return cast(_tasa_promedio as numeric(5,2));

end;

$BODY$
  LANGUAGE 'plpgsql' VOLATILE;
ALTER FUNCTION calcular_tasa_promedio(integer, date, date) OWNER TO cartera;