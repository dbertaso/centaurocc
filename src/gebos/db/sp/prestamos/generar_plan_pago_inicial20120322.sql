                ...
                begin
                  ...
                  _cuotas_gracia_diferidas = _prestamo.meses_diferidos / p_frecuencia_pago;
                    ...
                    */
                    ...
                    /*
                    if _programa.ultimo_desembolso = 1 or
                        _programa.ultimo_desembolso = 2 then

                        raise notice 'Entro por if programa.ultimo_desembolso ==========================>';
                        raise notice '==================================================================>';

                        _interes_cuota_aux = _interes_cuota;

                        if _numero_cuota = 1 then

                            raise notice 'Entro por numero cuota = 1 =====================================>';
                            raise notice '================================================================>';

                            raise notice 'Interes Cuota Auxiliar ====> %', _interes_cuota_aux;
                            select into _interes_cuota * from total_interes_gracia(_prestamo.id);
                            if _interes_cuota is null or _interes_cuota = 0 then
                               _interes_cuota = _interes_cuota_aux;
                            end if;

                            raise notice 'Total interes primera cuota ====> %', _interes_cuota;

                        else

                            _interes_cuota = _interes_cuota_aux;
                            raise notice 'Interes Cuota Normal ====> %', _interes_cuota;
                        end if;
                  ...
                  if p_formula_id = 2 then
                     ...
                     */
                     ...
                     if i = _cantidad_cuotas and _amortizado_acumulado <> (_prestamo.monto_liquidado + _prestamo.monto_liquidado_insumos + _prestamo.monto_sras_total + _prestamo.monto_gasto_total) then
                        update plan_pago_cuota set amortizado_acumulado = (_prestamo.monto_liquidado + _prestamo.monto_liquidado_insumos + _prestamo.monto_sras_total + _prestamo.monto_gasto_total)
                          where id = _row_plan_pago_cuota.id;
                     end if;
                  ...
                  --    raise notice '_fecha_revision_tasa%',_fecha_revision_tasa;
                    ...
                    if _inicial = true and p_proyectado = false then
                      ...
                      values
                                  (p_tasa_valor, _prestamo.id, _prestamo.tasa_id, _prestamo.fecha_liquidacion);
                    ...
                    if p_desembolso_id <> 0 then
                      ...
                      insert into factura (numero, monto, fecha, fecha_realizacion, desembolso_id, tipo, prestamo_id, monto_banco, monto_sras, monto_insumos, monto_gastos)
                        values ((select ultima_factura from parametro_general) + 1, p_monto_desembolso, p_fecha_evento,
                        p_fecha_evento, _desembolso.id, 'D', p_prestamo_id, p_monto_banco, p_monto_sras, p_monto_insumos, p_monto_gasto_total);
...
ALTER FUNCTION generar_plan_pago(boolean, integer, integer, integer, integer, character, double precision, double precision, date, double precision, integer, integer, double precision, integer, integer, integer, boolean, double precision, integer, integer, boolean, date, double precision) OWNER TO cartera;
