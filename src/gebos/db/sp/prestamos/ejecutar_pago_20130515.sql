-- Function: ejecutar_pago(integer, character varying[], integer, character, double precision, integer, date, date, character varying, double precision, integer, boolean, boolean, boolean, boolean, boolean, character varying, character varying, boolean, integer, character)

-- DROP FUNCTION ejecutar_pago(integer, character varying[], integer, character, double precision, integer, date, date, character varying, double precision, integer, boolean, boolean, boolean, boolean, boolean, character varying, character varying, boolean, integer, character);

CREATE OR REPLACE FUNCTION ejecutar_pago(p_cliente_id integer, p_cheques character varying[], p_prestamo_id integer, p_modalidad character, p_monto double precision, p_oficina_id integer, p_fecha_realizacion date, p_fecha date, p_numero_voucher character varying, p_monto_efectivo double precision, p_entidad_financiera_id integer, p_proceso_nocturno boolean, p_cancelacion_prestamo boolean, p_hay_cheques boolean, p_exonerar_mora boolean, p_consultoria_juridica boolean, p_observaciones_analista character varying, p_analista_nombre_completo character varying, p_recuperaciones boolean, p_cuenta_bcv_id integer, p_tipo_pago character DEFAULT 'P'::bpchar)
  RETURNS integer AS
$BODY$
  declare
    ...
    _factura_remanente_por_aplicar numeric(16,2) = 0;
    ...
    _causado_no_devengado numeric(16,2) = 0;
  ...
  begin
    ...
    */
    ...
    /*
    --------------------------------------------------
    Lectura para verificar si tiene un compromiso de
    pago establecido
    --------------------------------------------------
    */
    ...
    if found then
      ...
      if p_fecha_realizacion = _compromiso_pago.fecha_pago then
        _estatus_compromiso = 'V'; --Cumplido en la fecha del compromiso
      end if;
      
      if p_fecha_realizacion < _compromiso_pago.fecha_pago then
        _estatus_compromiso = 'A';  --Cumplido antes de fecha de compromiso
      end if;
      
      if p_fecha_realizacion > _compromiso_pago.fecha_pago and
         p_fecha_realizacion <= _compromiso_pago.fecha_limite_pago then
         
        _estatus_compromiso = 'R'; -- Cumplido dentro de la fecha limite de pago
      end if;
      
      if _remanente_por_aplicar < _compromiso_pago.monto_pago  then
        _estatus_compromiso = 'P'; -- Cumplido parcialmente
      end if;
      
      /*
      ---------------------------------------------------
      Actualización del registro del compromiso de pago
      ---------------------------------------------------
      */
      ...
      update
              compromiso_pago
      set
              monto_efectivamente_pago = _remanente_por_aplicar,
              estatus = _estatus_compromiso
      where
              id = _compromiso_pago_id;
      ...
      */
      ...
      if found then
        ...
        set
                ...
                porcentaje_promesas_cumplidas = _porcentaje_promesas_cumplidas,
                porcentaje_promesas_cumplidas_parcialmente = _porcentaje_promesas_cumplidas_parcialmente
        where
    ...
    if _prestamo.estatus = 'V' and 
      ...
      if _remanente_por_aplicar >= (_prestamo.deuda) then
        
        raise notice 'Entro por remanente por aplicar > saldo =========> % % ', _remanente_por_aplicar, _prestamo.saldo_insoluto;
        
        if _prestamo.causado_no_devengado > 0 then
        
          if _remanente_por_aplicar > _prestamo.causado_no_devengado then
          
            _remanente_por_aplicar = _remanente_por_aplicar - _prestamo.causado_no_devengado;
            _intereses_pagados = _prestamo.causado_no_devengado;
            _con_monto_ingreso_intereses = _prestamo.causado_no_devengado;
            _causado_no_devengado = 0;
          else
            _intereses_pagados = _remanente_por_aplicar;
            _causado_no_devengado = _causado_no_devengado - _remanente_por_aplicar;
            _con_monto_ingreso_intereses = _remanente_por_aplicar;
    ...
    */
    ...
    loop
      ...
      */
      ...
      _dias_atraso = _plan_pago_cuota.fecha - p_fecha_realizacion;
      
      insert 
      into 
              pago_cuota (  plan_pago_cuota_id, pago_prestamo_id,
                            monto, interes_corriente, interes_diferido,
                            interes_mora, interes_desembolso, capital, remanente_por_aplicar, estatus_pago,
                            fecha_vencimiento, fecha_pago, dias_atraso)
        values
                         (  _plan_pago_cuota.id, _pago_prestamo_id, _pago_cuota_total,
                            ...
                            _pago_real_capital, _remanente_por_aplicar, _estatus_pago,
                            _plan_pago_cuota.fecha, p_fecha_realizacion, _dias_atraso);
      ...
      _pago_cuota_id1 = currval('pago_cuota_id_seq');
      _plan_pago_cuota_id = _plan_pago_cuota.id;
    ...
    /*
    -------------------------------------------------------
    Si el remanente por aplicar es mayor que cero se cobran
    los intereses causados a la fecha antes de realizar el
    abono a capital
    -------------------------------------------------------
    */
    ...
    if _remanente_por_aplicar > 0 then
      ...
      if _prestamo.causado_no_devengado > 0 then
        ...
        if _remanente_por_aplicar > _prestamo.causado_no_devengado then
          _intereses_pagados = _intereses_pagados + _prestamo.causado_no_devengado;
          _pago_acumulado_interes_corriente = _pago_acumulado_interes_corriente + _prestamo.causado_no_devengado;
          _con_monto_ingreso_intereses = _con_monto_ingreso_intereses + _prestamo.causado_no_devengado;
          _remanente_por_aplicar = _remanente_por_aplicar - _prestamo.causado_no_devengado;
          _causado_no_devengado = 0;
            ...
            _intereses_pagados = _remanente_por_aplicar;
            _causado_no_devengado = _prestamo.causado_no_devengado - _remanente_por_aplicar;
            _pago_acumulado_interes_corriente = _pago_acumulado_interes_corriente + _remanente_por_aplicar;
            _con_monto_ingreso_intereses = _con_monto_ingreso_intereses + _remanente_por_aplicar;
            _remanente_por_aplicar = 0;

        end if; -- Fin if _remanente_por_aplicar > _prestamo.causado_no_devengado then
        
      end if;   -- Fin if _prestamo.causado_no_devengado > 0 then
    ...
    */
    ...
    update 
            prestamo
    set
            causado_no_devengado = _causado_no_devengado,
            capital_pagado = capital_pagado + _capital_pagado,
    ...
    */
    ...
    if p_modalidad = 'A' and
      ...
      if _remanente_por_aplicar > 0 then
        ...
        _factura_remanente_por_aplicar = _remanente_por_aplicar;
        _remanente_por_aplicar = 0;
    ...
    if _factura_remanente_por_aplicar > 0 then
      _remanente_por_aplicar = _factura_remanente_por_aplicar;
    end if;
    ...
    if _factura_remanente_por_aplicar > 0 then
      _remanente_por_aplicar = 0;
      _factura_remanente_por_aplicar = 0;
    end if;

    if _intereses_pagados > 0 then
    
      update 
              factura
      set
              pago_interes = _intereses_pagados
      where
              id = _factura_id;
