-- Function: calcular_cartera_prestamo(date, integer, integer)

-- DROP FUNCTION calcular_cartera_prestamo(date, integer, integer);

CREATE OR REPLACE FUNCTION calcular_cartera_prestamo(p_fecha date, p_oficina_id integer, p_ultimo bigint)
  RETURNS boolean AS
$BODY$
...
declare
    ...
    /*
    ----------------------------------------------------------------------
    Lectura de control_cierre para determinar el último prestamo cerrado
    ----------------------------------------------------------------------
    */
    ...
    */
    ...
    for
        ...
        from
                prestamo
        where
                prestamo.estatus in ('E', 'P') and
                prestamo.numero > p_ultimo and
    ...
    */
    ...
    for
            ...
            where
                    prestamo.estatus not in ('S', 'L', 'Q', 'R', 'C', 'F', 'A', 'X') and
                    dia_facturacion = cast(extract(day from p_fecha) as integer) and
                    prestamo.numero = p_ultimo
            order by
                    prestamo.numero;
        ...
        if _cuota1.id is not null then
            ...
            if cast(extract(day from _cuota1.fecha) as integer) = _prestamo.dia_facturacion then
                ...
                if _prestamo.estatus = 'V' and _prestamo.remanente_por_aplicar > 0 then
                    ...
                    for
                            select count(id) as cantidad from plan_pago_cuota
                    where
                            fecha > p_fecha and tipo_cuota = 'C' and
                    ...
                    --raise notice 'leyo cuotas por cobrar ++++++++++++++++++++ %', _prestamo.numero;
                        ...
                        if _row_cuotas_por_cobrar.cantidad >= 1  then
                            ...
                            if _producto.abono_cantidad_cuotas > 0 then
                                ...
                                if _cuota.id is null then
                                    ...
                                    where
                                            ...
                                            plan_pago_id in (select
                                                            ...
                                                            order by
                                                                    numero asc;
                                end if;     --> if _cuota.id is null then
                                ...
                                if
                                    (_cuota.valor_cuota * _producto.abono_cantidad_cuotas) <= _prestamo.remanente_por_aplicar then
                                        _tiene_abono = true;
                                end if;
                            else
                            ...
                            end if;         ---> if _producto.abono_cantidad_cuotas > 0 then

                        end if;     ---> if _row_cuotas_por_cobrar.cantidad >= 1  then
            ...
            if (_tasa_historico.tasa_cliente <> _prestamo.tasa_vigente) and
            ...
            if _fecha_revision_tasa is not null and
                _fecha_revision_tasa = p_fecha then
            ...
            if _prestamo.tasa_forzada = true and
                _prestamo.tasa_forzada_fecha_vigencia < p_fecha then
            ...
            */
            ...
            if _tiene_abono = true and
                _tiene_tasa = false then
            ...
            if _tiene_tasa = true then
                ...
                if _tiene_historico = true and
                    _prestamo.tasa_forzada = false then
                    ...
                    into
                            prestamo_tasa_historico
                                                (   tasa_foncrei,
                                                    ...
                                                    fecha)
                                        values
                                                (   _tasa_historico.tasa_foncrei,
                                                    _tasa_historico.tasa_intermediario,
            ...
            end if;     ---> if _tiene_tasa = true then
            ...
            end if;	--> if cast(extract(day from _cuota1.fecha) as integer) = _prestamo.dia_facturacion then

        end if; ---> if _cuota1.id is not null then
    ...
    loop
        ...
        if _contador = 10000 then
            ...
            update
                    control_cierre
            set
                    ultimo_prestamo = _prestamo.numero
            where
  ...
  COST 100;
ALTER FUNCTION calcular_cartera_prestamo(date, integer, bigint) OWNER TO cartera;
