      ...
      if _plan_pago_cuota.id is not null then
          ...
          if _plan_pago_cuota.saldo_insoluto > 0 then
            ...
            */
            ...
            else   ---> _remanente_por_aplicar  <= _plan_pago_cuota_ultima.saldo_insoluto then

              /*
              -----------------------------------------------------------------------------
              Remanente por aplicar mayor que saldo insoluto de la ultima cuota
              pagada, se cancela el prestamo y se actualiza el remanente por aplicar
              en el prestamo
              -----------------------------------------------------------------------------
              */

              _abono_capital = _plan_pago_cuota.saldo_insoluto;
              raise notice 'Abono a Capital       %', _abono_capital;
              _estatus = 'C';

              raise notice 'Entro por remanente por aplicar > saldo ', _estatus;
              ...
              _remanente_por_aplicar = _remanente_por_aplicar - _plan_pago_cuota.saldo_insoluto;
              raise notice 'Remanente por aplicar despues de calcular cuando es mayor que el saldo insoluto       %', _remanente_por_aplicar;
      ...
      if _plan_pago_cuota.saldo_insoluto is null then
         ...
         if found then
           ...
           if _plan_pago_cuota.saldo_insoluto > 0 then
              ...
              RAISE NOTICE 'Entro por ultima cuota saldo_insoluto > 0   %', _estatus;
              ...
              */
              ...
              if _remanente_por_aplicar  <= _plan_pago_cuota.saldo_insoluto then
                 ...
                 */
                 ...
                 RAISE NOTICE 'Estatus pago incompleto = false remanente por aplicar > saldo cuota is null   %', _estatus;
                 RAISE INFO 'Estatus pago incompleto = false remanente por aplicar > saldo cuota is null   %', _estatus;
      ...
      */
      ...
      if  not (_remanente_por_aplicar =  _remanente_por_aplicar_inicial and p_proceso_nocturno = true ) then
        ...
        perform generar_factura((select ultima_factura from parametro_general) + 1, _monto_factura, p_fecha,
                                 ...
                                 p_consultoria_juridica, _abono_capital);
...
ALTER FUNCTION ejecutar_pago(integer, character varying[], integer, character, double precision, integer, date, date, character varying, double precision, integer, boolean, boolean, boolean, boolean, boolean,varchar,varchar(50)) OWNER TO cartera;
