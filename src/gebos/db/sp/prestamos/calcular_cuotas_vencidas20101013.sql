  ...
  begin
    ...
    */
    ...
    open _cur_plan_pago_cuota for
       select
              sum((valor_cuota + interes_desembolso + cuota_extra)-(pago_interes_corriente + pago_capital + pago_interes_diferido + pago_interes_desembolso + pago_cuota_extra)) as total_monto,
   ...
   if found then
     ...
     if _plan_pago_cuota.total_monto is not null then
        _cantidad_cuotas_vencidas = _cantidad_cuotas_vencidas + _plan_pago_cuota.total_cantidad;
        _monto_cuotas_vencidas = _monto_cuotas_vencidas +  _plan_pago_cuota.total_monto;
         --raise notice 'Monto Cuotas Vencidas Amortizacion    ==============> %', _monto_cuotas_vencidas;
         --raise notice 'Monto Cuotas Vencidas Amortizacion DB ==============> %', _plan_pago_cuota.total_monto;
