  ...
  begin
    ...
    */
    ...
    if _prestamo.estatus = 'V' and 
       _prestamo.cantidad_cuotas_vencidas = 0 and
       _remanente_por_aplicar > (_prestamo.saldo_insoluto + _prestamo.interes_diferido_por_vencer + _prestamo.causado_no_devengado) then
    
      /*
      -------------------------------------------------------------------
      Recorrido de cuotas no vencidas para cancelar el interés diferido
      -------------------------------------------------------------------
      */
      ...
      /*
      -----------------------------------------------------------------------------
      Apertura del cursor de plan_pago_cuota para cancelación de interés diferido
      -----------------------------------------------------------------------------
      */
      ...
      if found then
        ...
        loop
          ...
          exit when 
            ...
            raise notice 'Paso por loop de cuotas en cuotas vencidas = 0 <-------> %  <--> Remanente <---> %', _plan_pago_cuota.id, _remanente_por_aplicar;

            if (_remanente_por_aplicar > 0) then

              if (_plan_pago_cuota.interes_diferido > 0) then
              
                if (_remanente_por_aplicar > (_plan_pago_cuota.interes_diferido - _plan_pago_cuota.pago_interes_diferido)) then

                  _remanente_por_aplicar = _remanente_por_aplicar - (_plan_pago_cuota.interes_diferido - _plan_pago_cuota.pago_interes_diferido);
                  _pago_interes_diferido = (_plan_pago_cuota.interes_diferido - _plan_pago_cuota.pago_interes_diferido);
                else
                  _pago_interes_diferido = _remanente_por_aplicar;
                  _remanente_por_aplicar = 0;

                end if;         ---> Fin if (_remanente_por_aplicar > _plan_pago_cuota.interes_diferido) then
                ...
                /*
                -----------------------------------
                Actualización de plan_pago_cuota
                -----------------------------------
                */

                raise notice 'Pago interes diferido <-------> %  <--> intereses_pagados <---> %  <----> remanente <----> % ', _plan_pago_cuota.id, _pago_interes_diferido, _remanente_por_aplicar;
              ...
              end if;           ---> Fin if (_plan_pago_cuota.interes_diferido > 0) then
      ...
      /*
      --------------------------------------------------------------------
      Se verifica si se adeudan intereses causados para su cancelación
      --------------------------------------------------------------------
      */

      if _remanente_por_aplicar > 0 and
         _prestamo.causado_no_devengado > 0 then
        
        if _remanente_por_aplicar >= _prestamo.causado_no_devengado then
       
          _interes_causado_pagado = _interes_causado_pagado + _prestamo.causado_no_devengado;
          _remanente_por_aplicar = _remanente_por_aplicar - _prestamo.causado_no_devengado;
      
        else
          _interes_causado_pagado = _remanente_por_aplicar;
          _remanente_por_aplicar = 0;
        ...
        update
                prestamo
        set
                causado_no_devengado = causado_no_devengado - _interes_causado_pagado
        where
                id = _prestamo.id;
      ...
      /*
      ------------------------------------------------------------------------
      Se verifica si el _remanente_por_aplicar es mayor que el saldo para
      proceder a la cancelación del financiamiento
      ------------------------------------------------------------------------
      */
      ...
      if _remanente_por_aplicar >= _prestamo.saldo_insoluto then
        ...
        /*
        ------------------------------------------------------------------------------
        Actualización del estatus pago de las cuotas después de cancelar el prestamo
        ------------------------------------------------------------------------------
        */
    ...
    if not found and 
       ...
       _prestamo.causado_no_devengado > 0 then
        ...
        if _remanente_por_aplicar >= _prestamo.causado_no_devengado then
          ...
          _interes_causado_pagado = _remanente_por_aplicar - _prestamo.causado_no_devengado;
    ...
    */
    ...
    perform generar_factura((select ultima_factura from parametro_general) + 1, _monto_factura, p_fecha,
                             ...
                             p_consultoria_juridica, _abono_capital, p_recuperaciones, _intereses_pagados);
