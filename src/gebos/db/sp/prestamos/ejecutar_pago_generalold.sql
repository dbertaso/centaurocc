  ...
  --for i in array_lower(p_prestamos,1) .. array_upper(p_prestamos,1) loop  
    ...
    --   _pago_interes_corriente + _pago_capital + _remanente_por_aplicar_cuota;
      ...
      update plan_pago_cuota set 
        pago_interes_diferido = _pago_interes_diferido,
        pago_interes_mora = _pago_interes_mora,
        pago_interes_desembolso = _pago_interes_desembolso,
        pago_interes_corriente = _pago_interes_corriente,
        pago_capital = _pago_capital,
...
--    if _anio_1 = 0 or _anio_1 = extract (year from _plan_pago_cuota.fecha) then
 ...
 if p_cancelacion_prestamo = false then
	 if p_proceso_nocturno = false then
		  -- INIIO CONTABILIDAD
		  if _anio_2 = 0 then
		     raise notice 'ENTRO POR PAGO AV______________';
			  perform stc_pago(
