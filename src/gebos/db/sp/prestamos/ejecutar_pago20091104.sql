    ...
    --_pago_monto_prestamo  < _prestamo.exigible then
      ...
      */
      ...
      if  not (_remanente_por_aplicar =  _remanente_por_aplicar_inicial and p_proceso_nocturno = true ) then
        ...
        insert into factura
                            (numero, monto, fecha, fecha_realizacion, pago_cliente_id,
                             tipo, proceso_nocturno, prestamo_id, remanente_por_aplicar)
                    values
                            ((select ultima_factura from parametro_general) + 1, _monto_factura, p_fecha,
                             p_fecha_realizacion, _pago_cliente_id, 'P', p_proceso_nocturno, _prestamo_id, _remanente_por_aplicar) ;
