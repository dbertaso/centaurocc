-- Function: generar_plan_pago(boolean, integer, integer, integer, integer, bpchar, double precision, double precision, date, double precision, integer, integer, double precision, integer, integer, integer, boolean, double precision, integer, integer, boolean, date, double precision, numeric)

-- DROP FUNCTION generar_plan_pago(boolean, integer, integer, integer, integer, bpchar, double precision, double precision, date, double precision, integer, integer, double precision, integer, integer, integer, boolean, double precision, integer, integer, boolean, date, double precision, numeric);

CREATE OR REPLACE FUNCTION generar_plan_pago(p_proyectado boolean, p_formula_id integer, p_prestamo_id integer, p_plan_pago_id integer, p_numero_cuota_inicial integer, p_tipo_cuota_inicial bpchar, p_interes_acumulado_inicial double precision, p_amortizado_acumulado_inicial double precision, p_fecha_liquidacion date, p_monto double precision, p_plazo integer, p_frecuencia_pago integer, p_tasa_valor double precision, p_meses_muertos integer, p_meses_gracia integer, p_meses_diferidos integer, p_exonerar_intereses_diferidos boolean, p_tasa_valor_gracia double precision, p_frecuencia_pago_gracia integer, p_desembolso_id integer, p_inicial boolean, p_fecha_evento date, p_monto_desembolso double precision, p_interes_diferido numeric)
  RETURNS boolean AS
$BODY$
...
begin
  ...
  loop 
    ...
    if _prestamo.intermediado = true then
      ...
      _interes_foncrei = _interes_corriente_ajustado * (_programa.porcentaje_tasa_foncrei/100);
  ...
  --    raise notice '_fecha_revision_tasa%',_fecha_revision_tasa;
    ...
    if _inicial = true and p_proyectado = false then   
      ...
      select into _tasa_historico * from tasa_historico
        ...
        where tasa_valor_id in (select id from tasa_valor where tasa_id = _prestamo.tasa_id and programa_id = _programa.id) order by id desc limit 1; 
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION generar_plan_pago(boolean, integer, integer, integer, integer, bpchar, double precision, double precision, date, double precision, integer, integer, double precision, integer, integer, integer, boolean, double precision, integer, integer, boolean, date, double precision, numeric) OWNER TO cartera;
