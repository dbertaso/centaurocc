/*****************************************************************************************************
 ...
 Función que ejecuta el cierre de cartera por programa (Calculo de Devengos, cuotas vencidas, cambios
 de tasa).
 Params:
 @p_fecha         Fecha de Proceso data por la tabla control cierre
 @p_oficina_id    Oficina que origina el proceso
 @p_programa_id   Programa al que se procesará el cierre
...
CREATE OR REPLACE FUNCTION calcular_cartera_programa(p_fecha date, p_oficina_id integer, p_programa_id integer, p_ultimo bigint)
  RETURNS boolean AS
$BODY$
                    declare
                    ...
                    begin
                      ...
                      raise info '[GEBOS] calcular_cartera_financiamiento {p_fecha: %, p_oficina_id: %, p_programa_id: %', p_fecha, p_oficina_id, p_programa_id;
                      ...
                      */
                      open _cur_prestamo for select * from
                        ...
                        where prestamo.estatus not in ('S', 'Q', 'R', 'C', 'F', 'A', 'X','K') order by prestamo.numero;
                      ...
                      --raise debug 'PRESTAMO NUMERO ++++++++++++++++++++ %', _prestamo.numero;
                      ...
                      */
                      ...
                      open _cur_prestamo for select * from prestamo inner join solicitud on (prestamo.solicitud_id = solicitud.id and
                                       solicitud.programa_id = p_programa_id)
                      where
                            prestamo.estatus not in ('S', 'Q', 'R', 'C', 'F', 'A', 'X', 'K')
                      ...
                      */
                      ...
                      --raise debug 'plan pago del prestamo +++++++%', _plan.id;
                      --raise debug 'fecha proceso +++++++%', p_fecha;
                     ...
                     if _cuota1.id is not null then
                        ...
                        --raise debug 'entro en el segundo if +++++++%', _prestamo.numero;

                      ---> fin del cambio


                      --raise debug 'estatus ++++++++++++++++++++ %', _prestamo.estatus;
                      --raise debug 'remanente por aplicar ++++++++++++++++++++ %', _prestamo.remanente_por_aplicar;
                      ...
                      --raise debug 'Entro a _prestamo.estatus = V and _prestamo.remanente_por_aplicar > 0 ++++++++++++++++++++ %', _prestamo.remanente_por_aplicar;
                        ...
                        fetch _cur_cuotas_por_cobrar INTO _row_cuotas_por_cobrar;
                                    ...
                                    --raise debug 'leyo cuotas por cobrar ++++++++++++++++++++ %', _prestamo.numero;

                        if _row_cuotas_por_cobrar.cantidad >= 1  then

                          --raise debug 'paso por cantidad de cuotas a cobrar mayor o igual que 1++++++++++++++++++++ %', _prestamo.numero;
                      ...
                      _valor_tasa = 0;
                        ...
                        select into _tasa_historico * from tasa_valor
                          where tasa_valor.tasa_id = _prestamo.tasa_id and
                      ...
                      */
                      ...
                      /* Fin de modificacion validacion ultima cuota */
                        ...
                        --raise debug 'Tiene abono = true y tiene tasa = false ++++++++++++++++++++ %', _prestamo.numero;
                            ...
                            insert into prestamo_tasa_historico
                                  ...
                                  values(_tasa_historico.valor, _prestamo.id, _tasa_historico.tasa_id, p_fecha);
                            else
                              insert into prestamo_tasa_historico
                                  (tasa_cliente, prestamo_id, tasa_id, fecha)
                                  values(_valor_tasa, _prestamo.id, null, p_fecha);
                              end if;
                            end if;
                            if _tiene_abono = true then
                      ...
                      where prestamo.estatus not in ('S', 'Q', 'R', 'C', 'F', 'A', 'X','K') and
                                  prestamo.senal_visita = true
                      ...
                      --raise debug 'Ejecutando ultimo loop ++++++++++++++++++++ %', _prestamo.numero;
                      ...
                      perform calcular_capital_vencido(_prestamo.id, p_fecha, false);
                      --perform calcular_interes_causado(_prestamo.id, p_fecha, false);
                      --perform calcular_interes_gracia(_prestamo.id, p_fecha, false);

                      -- Agregado por AB. Ajustes de intereses por cambio de tasa
                      --perform ajustar_intereses(_prestamo.id, p_fecha);
                      -- Agregado por AB. Calculo de devengo de intereses Ordinarios (O)
                      --perform calcular_interes_devengado(_prestamo.id, p_fecha, 'O');
                      ...
                      */
                      ...
                      raise info '[GEBOS] calcular_cartera_programa finalizado';
                      return true;
...
GRANT EXECUTE ON FUNCTION calcular_cartera_programa(date, integer, integer) TO cartera;
GRANT EXECUTE ON FUNCTION calcular_cartera_programa(date, integer, integer) TO public;
