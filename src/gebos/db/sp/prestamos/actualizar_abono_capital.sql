CREATE OR REPLACE FUNCTION actualizar_abono_capital(p_prestamo_id integer, p_fecha date) RETURNS boolean AS
$body$
declare
  ...
  */
  ...
  if _prestamo.abono_capital > 0 and
    ...
    perform generar_plan_pago_evento(false, _prestamo.id, _fecha_inicio,
                                     false, 0, false, 0, true, _prestamo.abono_capital, 0, 0);
  ...
  if _prestamo.abono_capital > 0 and
     ...
     */
     ...
     if _prestamo.abono_capital < _prestamo.saldo_insoluto then
        ...
        _remanente_por_aplicar = _prestamo.abono_capital - _prestamo_saldo_insoluto;
     end if;
