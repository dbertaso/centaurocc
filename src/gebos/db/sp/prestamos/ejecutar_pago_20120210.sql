...
$BODY$
    ...
    --_pago_monto_prestamo  < _prestamo.exigible then
      ...
      if _prestamo.estatus = 'V' and 
        ...
        if _remanente_por_aplicar > _prestamo.saldo_insoluto then
          ...
          /* Lectura del prestamo despues de actualizar a cancelado */
     ...
     -- select into _factura * from factura order by id desc limit 1;
      ...
      raise notice 'Solicitud id --------> % ',_factura_id;
  ...
  COST 100;
ALTER FUNCTION ejecutar_pago(integer, character varying[], integer, character, double precision, integer, date, date, character varying, double precision, integer, boolean, boolean, boolean, boolean, boolean, character varying, character varying, boolean, character)
  OWNER TO cartera;
