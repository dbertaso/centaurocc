...
$BODY$
    ...
    begin
      ...
      loop
        ...
        if _plan_pago_cuota.tipo_cuota = 'C' and _contador = 1 then
      ...
      if _vencido = true then
        ...
        if p_proyeccion = false then
          ...
          */
          ...
          raise notice 'Actualizando monto mora en el prestamo ========> %', p_prestamo_id;
        ...
        */
        ...
        if _cantidad_cuotas_mora <= 0 and _prestamo.estatus not in ('J','L','K') then
            _estado = 'V';
        end if;

        if ( _cantidad_cuotas_mora > 0 and
            _cantidad_cuotas_mora < _parametro_general.cuotas_pase_vencido) and
            _prestamo.estatus not in ('J','L','K') then

         _estado = 'H';
       end if;

        if ( _cantidad_cuotas_mora >= _parametro_general.cuotas_pase_vencido) and
             _prestamo.estatus not in ('J','L','K') then
