...
--select id from prestamo where numero = 8000033665;
--select calcular_saldo_insoluto(8726,'2008/09/11',true)
...
$BODY$
 ...
 begin
   ...
   into _saldo_insoluto
		...
		(sum(plan_pago_cuota.amortizado) - sum(plan_pago_cuota.pago_capital)) as saldo_insoluto
   from
		plan_pago_cuota
   where
		estatus_pago <> 'T' and
