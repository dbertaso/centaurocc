...
$BODY$
    ...
    begin
      ...
      /*
      ---------------------------------------------
      Busca el ultimo registro de control_cierre
      ---------------------------------------------
      */

      select into _control_cierre *
      from
            control_cierre
      order by
            fecha_proceso desc limit 1;
      ...
      */
      ...
      open _cur_prestamo for select * from
        ...
        where prestamo.estatus not in ('S', 'L', 'Q', 'R', 'C', 'F', 'A', 'X') and
                prestamo.numero > _control_cierre.ultimo_prestamo
      ...
      */
      ...
      for
            ...
            from
                    prestamo
                            inner join solicitud on (prestamo.solicitud_id = solicitud.id and
                                                     solicitud.programa_id = p_programa_id)
            where
                    prestamo.estatus in ('E', 'P') and
                    prestamo.numero > _control_cierre.ultimo_prestamo and
      ...
      loop
         ...
         */
         ...
         perform iniciar_transaccion(
                                      ...
                                      'p_pago_ordinario',
                                      'L',
                                      'Pago Ordinario (Cierre) para el pr̩stamo N̼mero '||_prestamo.numero);
      ...
      */
      ...
      where
        ...
        and prestamo.numero > _control_cierre.ultimo_prestamo
        order by prestamo.numero;
