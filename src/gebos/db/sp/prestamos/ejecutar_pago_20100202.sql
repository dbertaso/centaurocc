-- Function: ejecutar_pago(integer, character varying[], integer, character, double precision, integer, date, date, character varying, double precision, integer, boolean, boolean, boolean, boolean)

-- DROP FUNCTION ejecutar_pago(integer, character varying[], integer, character, double precision, integer, date, date, character varying, double precision, integer, boolean, boolean, boolean, boolean);
...
$BODY$
  ...
  declare
    ...
    begin
     ...
     insert into pago_cliente
                             (fecha, modalidad, monto, cliente_id, oficina_id, fecha_realizacion, numero_voucher, entidad_financiera_id, efectivo)
                 values
                             (p_fecha, p_modalidad, p_monto, p_cliente_id, p_oficina_id, p_fecha_realizacion, p_numero_voucher, p_entidad_financiera_id, p_monto_efectivo);
   ...
   -- Inserta las pago_forma
					  ...
					  _sector_economico = (SELECT get_datos_cliente(_prestamo.cliente_id, 'sector_economico'));
                                          _estado_prestamo = (SELECT resolver_estatus(_prestamo.estatus));
 					  _distincion_cobranza = (SELECT get_tipo_pago(_pago_cliente_id));
                                          _analista = p_analista_nombre_completo;


                                          insert into factura (numero, monto, fecha, fecha_realizacion, pago_cliente_id, tipo, proceso_nocturno, prestamo_id, remanente_por_aplicar, tipo_cartera_id, codigo_contable, sector_economico, estado, comentario_analista, distincion_cobranza, analista, consultoria_juridica)
                                            values ((select ultima_factura from parametro_general) + 1, _monto_factura, p_fecha,
                                            p_fecha_realizacion, _pago_cliente_id, 'P', p_proceso_nocturno, _prestamo_id, _remanente_por_aplicar, _prestamo.tipo_cartera_id, _prestamo.codigo_contable, _sector_economico, _estado_prestamo, p_observaciones_analista, _distincion_cobranza, _analista, p_consultoria_juridica ) ;
                                       ...
                                       if p_cancelacion_prestamo = false then
                                         ...
                                         else			--> if p_proceso_nocturno = false then
  ...
  COST 100;
ALTER FUNCTION ejecutar_pago(integer, character varying[], integer, character, double precision, integer, date, date, character varying, double precision, integer, boolean, boolean, boolean, boolean) OWNER TO cartera;
