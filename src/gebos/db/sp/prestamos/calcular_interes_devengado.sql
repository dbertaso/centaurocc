...
begin
	...
	*/
	  ...
	  where
	    ...
	    and fecha - p_fecha_evento <=30 * _prestamo.frecuencia_pago
	  order by fecha
	  limit 1;
   ...
   --Fecha de vencimiento de la cuota (cierra al dia anterior). Calculo último tramo por diferencia
   if (p_fecha_evento = _cuota_siguiente.fecha - 1) then
        ...
        if p_tipo_interes = 'O' then
          ...
          _interes_causado = _cuota_siguiente.interes_corriente + interes_diferido;
