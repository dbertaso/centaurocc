...
declare
  ...
  begin
   ...
   if found then
     ...
     if p_proyeccion = false then
       ...
       if _plan_pago_cuota.total_cantidad <= 0 then
          _estado = 'V';
       end if;

       if (_plan_pago_cuota.total_cantidad > 0 and
           _plan_pago_cuota.total_cantidad < _parametro.cuotas_pase_vencido) then
       ...
       if _plan_pago_cuota.total_cantidad >= _parametro.cuotas_pase_vencido then
   ...
   */
   ...
   _cuotas_pendientes = _total_cuotas - _cuotas_pagadas;
