...
begin
  ...
  _anio_1 = extract (year from p_fecha_liquidacion);
  ...
  end loop;       ---> for i in 1..p_meses_muertos loop
  ...
  for i in 1.._cantidad_cuotas loop
    ...
    if _prestamo.tipo_diferimiento = true and
       _prestamo.meses_diferidos > 0 then
    ...
    end if;      ---> Fin if _prestamo.tipo_diferimiento = true then
  ...
  from
          plan_pago_cuota
  where
          plan_pago_id = _plan_pago_id and tipo_cuota = 'G';
  ...
  end if;     ---> Fin if _factura_id <> 0 then
