CREATE OR REPLACE FUNCTION cancelar_prestamo(p_cliente_id integer,
                                             ...
                                             p_observacion_precancelacion text,
...
declare
    ...
    _anio_1 integer = 0;
    _pago_capital numeric(16,2) = 0;
    _monto_total numeric(16,2) = 0;
    ...
    */
        ...
        */
         ...
         */
         ...
         loop
           ...
           /*
		       -----------------------------------------------
           Actualiza el pago de capital en la cuota
           -----------------------------------------------
           */
           ...
           */
           ...
           raise notice 'CAPITAL PAGADO ACTUALIZADO ==========> %', _pago_capital;
         ...
         loop
           ...
           */
               ...
               if _plan_pago_cuota.tipo_cuota = 'G' then
                 ...
                 if _plan_pago_cuota.estatus_pago <> 'D' then
           ...
           */
		       ...
		       select
               into
                    _pago_interes_diferido, _pago_capital
                    pago_interes_diferido, pago_capital
		       from
		              plan_pago_cuota
		       where
		              id = _plan_pago_cuota.id;
           ...
           */
           ...
           if _prestamo.codigo_d3 is not null then
              ...
              _monto_total = _pago_interes_diferido + _pago_capital;
           else
             if _plan_pago_cuota.tipo_cuota = 'C' then
		           _pago_acumulado_interes_diferido = _pago_acumulado_interes_diferido + _pago_interes_diferido;
               _monto_total = _pago_interes_diferido + _pago_capital;
           ...
           */
           ...
           if _prestamo.codigo_d3 is null then
     ...
     */
      ...
      */
      ...
      /*
      --------------------------------------
      Creación de cursor de cuotas pagadas
      --------------------------------------
      */

	  open       _con_cur_pago_cuota
      for select
	        ...
	        from
                 pago_cuota inner join plan_pago_cuota
                                       on (plan_pago_cuota.id = pago_cuota.plan_pago_cuota_id)

          where
                 pago_cuota.pago_prestamo_id = _pago_prestamo.id;

      /*
      ----------------------------------------
      Recorrido del cursor de cuotas pagadas
      ----------------------------------------
      */
    ...
    /*
    -----------------------------------------
    Actualización del estatus en la factura
    -----------------------------------------
    */
