...
CREATE OR REPLACE FUNCTION generar_plan_pago_evento(p_proyectado boolean, p_prestamo_id integer, p_fecha_evento date, p_tiene_desembolso boolean, p_monto_desembolso double precision, p_tiene_tasa boolean, p_valor_tasa double precision, p_tiene_abono boolean, p_monto_abono double precision, p_desembolso_id integer, p_extension_meses_muertos integer)
  RETURNS boolean AS
$BODY$
  ...
  COST 100;
ALTER FUNCTION generar_plan_pago_evento(boolean, integer, date, boolean, double precision, boolean, double precision, boolean, double precision, integer, integer) OWNER TO cartera;
