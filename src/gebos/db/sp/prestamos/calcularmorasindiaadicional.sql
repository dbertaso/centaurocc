-- Function: calcular_mora(integer, date, date, boolean)

-- DROP FUNCTION calcular_mora(integer, date, date, boolean);

CREATE OR REPLACE FUNCTION calcular_mora(p_prestamo_id integer, p_fecha_evento date, p_fecha_auxiliar date, p_proyeccion boolean)
  RETURNS boolean AS
$BODY$
declare
		...
		_plan_pago_mora_id = currval('plan_pago_mora_id_seq'); 
	     ...
	     -- end if;
		  ...
		  if _base_calculo_intereses = 360 then
	  	    _dias_mora = (calcular_dias_360(_plan_pago_mora.fecha_inicio, _plan_pago_mora.fecha_fin));
	  	    ...
	  	    _dias_mora = (  _plan_pago_mora.fecha_fin - _plan_pago_mora.fecha_inicio);
	  	  end if;	
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION calcular_mora(integer, date, date, boolean) OWNER TO cartera;
