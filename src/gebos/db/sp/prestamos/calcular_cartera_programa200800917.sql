...
declare
  ...
  _programa programa%rowtype;		---> db 20071029
   _plan1 plan_pago%rowtype;		---> db 20080910
...
begin
  ...
  */
  ...
  loop
	...
	perform calcular_mora(_prestamo.id, p_fecha, p_fecha, false,1);
  ...
  */
  ...
  open _cur_prestamo for select * from
    prestamo inner join solicitud on (prestamo.solicitud_id = solicitud.id and
				      solicitud.programa_id = p_programa_id)
	where
		prestamo.estatus not in ('S', 'L', 'Q', 'R', 'C', 'F', 'A', 'X','K') and
  ...
  */
  ...
  open _cur_prestamo for select * from prestamo inner join solicitud on (prestamo.solicitud_id = solicitud.id and
									 solicitud.programa_id = p_programa_id)
	where
		    prestamo.estatus not in ('S', 'L', 'Q', 'R', 'C', 'F', 'A', 'X','K')
  ...
  loop
	...
	*/
	...
	--raise notice 'id de la cuota +++++++%', _cuota1.id;
		...
		---> fin del cambio
				...
				fetch _cur_cuotas_por_cobrar INTO _row_cuotas_por_cobrar;
					...
					if _row_cuotas_por_cobrar.cantidad >= 1  then
						...
						if _producto.abono_cantidad_cuotas > 0 then
							...
							select into _cuota * 	from plan_pago_cuota
										where estatus_pago = 'X' and plan_pago_id in (select
																	id
																from
																	plan_pago
																where
					...
					end if;	--> if _row_cuotas_por_cobrar.cantidad >= 1
			...
			end if;	---> if _prestamo.estatus = 'V' and _prestamo.remanente_por_aplicar > 0
			...
			select into _tasa_historico * from tasa_historico
							where tasa_valor_id in (select id from tasa_valor where tasa_id = _prestamo.tasa_id)
										order by id desc limit 1;

			if (_tasa_historico.tasa_cliente <> _prestamo.tasa_vigente)  and _prestamo.tasa_fija = false and
			   ((_fecha_revision_tasa is null) or (_fecha_revision_tasa is not null and _fecha_revision_tasa = p_fecha))  then
			...
			*/
			...
			if _tiene_abono = true and _tiene_tasa = false then
				...
				if remanente_por_aplicar <= _prestamo.saldo_insoluto then
					...
					_prestamo.remanente_por_aplicar = _prestamo.remanente_por_aplicar - _prestamo.saldo_insoluto;
					update prestamo set saldo_insoluto = 0,
					                    remanente_por_aplicar = _prestamo.remanente_por_aplicar,
							    estatus = 'C'
					where id = _prestamo.id;
				end if;
			...
			if _tiene_tasa = true then
				...
				if _tiene_historico = true and _prestamo.tasa_forzada = false then
				    insert into prestamo_tasa_historico
								...
								values
									( _tasa_historico.tasa_foncrei, _tasa_historico.tasa_intermediario,
									  _tasa_historico.tasa_cliente, _prestamo.id, _tasa_historico.id, p_fecha);
			      else
				    insert into prestamo_tasa_historico
							(tasa_foncrei, tasa_intermediario, tasa_cliente, prestamo_id, tasa_historico_id, fecha)
						values
							(_valor_tasa, 0, _valor_tasa, _prestamo.id, null, p_fecha);
				end if;

			end if;   --> if _tiene_tasa = true

			if _tiene_abono = true then

				--raise notice 'Actualizo remanente por aplicar = 0 ++++++++++++++++++++ %', _prestamo.numero;
				if _prestamo.remanente_por_aplicar = 0 then
	...
	*/
	...
	_tiene_abono = true then
		...
		/*
		-----------------------------------------------------------------------------------
		Si el prestamo intermediado se iguala el interes corriente con el interes foncrei
		-----------------------------------------------------------------------------------


		if _prestamo.intermediado = true then

			select into _plan1 * from plan_pago where prestamo_id = _prestamo.id and
			                                          activo = true;
			...
			if found then
				...
				where
					plan_pago_cuota.plan_pago_id = _plan1.id and
  ...
  end loop; --> fin loop _cur_prestamo
  ...
  open _cur_prestamo for select * from
		...
		where prestamo.estatus not in ('S', 'L', 'Q', 'R', 'C', 'F', 'A', 'X','K') and
		      prestamo.senal_visita = true
