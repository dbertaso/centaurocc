-- Function: calcular_mora(p_prestamo_id int4, p_fecha_evento date, p_fecha_auxiliar date, p_proyeccion bool)

-- DROP FUNCTION calcular_mora(p_prestamo_id int4, p_fecha_evento date, p_fecha_auxiliar date, p_proyeccion bool);

CREATE OR REPLACE FUNCTION calcular_mora(p_prestamo_id int4, p_fecha_evento date, p_fecha_auxiliar date, p_proyeccion bool)
  RETURNS bool AS
$BODY$
declare
...
--	    	update prestamo set dias_mora = _dias_mora_total, monto_mora =  _intereses_mora_total - _pago_interes_mora, estatus = 'E' where id = p_prestamo_id;
			...
			/*
			-----------------------------------------------------------------------
			Se actualizan los dias de mora y monto de mora en cero en el prestamo
			antes de recalcular los dias y el monto despues del pago

			Diego Bertaso	2008-03-12
			-----------------------------------------------------------------------
			*/
...
ALTER FUNCTION calcular_mora(p_prestamo_id int4, p_fecha_evento date, p_fecha_auxiliar date, p_proyeccion bool) OWNER TO cartera;
